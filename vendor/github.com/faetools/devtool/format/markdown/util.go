...
func writeFormattedLines(w util.BufWriter, source []byte, n ast.Node, lang string) {
	...
	tw := writers.NewTrimWriter(w, sNewLine)
	if len(lang) == 0 {
		// no formatting necessary
		writeLines(tw, source, n)
		return
	}

	b := &bytes.Buffer{}
	writeLines(b, source, n)
	_, _ = tw.Write(formatCode(lang, b.Bytes()))
}
...
func (r *Renderer) renderChildren(w io.Writer, source []byte, n ast.Node) error {
